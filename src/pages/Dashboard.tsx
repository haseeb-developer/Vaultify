...
import { Plus, Lock, Trash2, Save, ShieldOff, Search, Bold, Italic, Underline, Heading, PaintBucket, X, Folder as FolderIcon, Palette, Tag, Edit3, XCircle, Check, Copy, Star, ChevronDown, ChevronRight, Info } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Shift+S save shortcut
    function handleKeyDown(e: KeyboardEvent) {
      if (e.shiftKey && (e.key === 's' || e.key === 'S')) {
        if (currentNote && editor) {
          e.preventDefault();
          handleSaveNote(true);
          toast.success('Note saved (Shift+S)!');
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const handleSelectNote = (note: Note) => {
      ...
      // If locked, remove lastNoteId from localStorage
  ...
  // When note lock status changes, update localStorage
  useEffect(() => {
    if (currentNote && !currentNote.isLocked) {
      localStorage.setItem('lastNoteId', currentNote.id);
    } else {
      localStorage.removeItem('lastNoteId');
    }
  }, [currentNote?.id, currentNote?.isLocked]);

  // On mount, restore last opened note if available and not locked
  useEffect(() => {
        ...
        <>
            ...
            <button onClick={handleCreateFolder} className="action-btn px-5 py-2 flex items-center gap-2 shadow-lg hover:scale-105 active:scale-95 transition-all "><Plus size={20} /> New Folder</button>
              ...
              <Plus size={20} />
                    ...
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleNoteDragEnd}>
                        <SortableContext items={filteredNotes.map(n => n.id)} strategy={verticalListSortingStrategy}>
                          ...
                          {(note.tags && note.tags.length > 0) ? (
                            <div className="note-card-tags-count">{note.tags.length} tag{note.tags.length > 1 ? 's' : ''}</div>
                          ) : (
                            <div className="note-card-tags-count no-tags">No Tags Found</div>
                          )}
                ...
                <main className="editor-main flex-1 w-full min-w-0 bg-gradient-to-br from-white via-blue-50 to-blue-100 rounded-t-3xl lg:rounded-3xl shadow-2xl p-2 sm:p-4 md:p-6 xl:p-8 flex flex-col gap-6 animate-fadeIn overflow-x-auto ">
                  ...
                  <button onClick={() => handleDeleteNote(currentNoteId!)} className="action-btn danger"><Trash2 size={14} /> Delete</button>
                                ...
                                {/* Auto-save indicator */}
                {/* <span className="text-base font-bold px-3 py-1 rounded-full bg-gradient-to-r from-green-400 to-blue-400 text-white shadow animate-pulse">Saved</span> */}
                            </div>
              {/* Tag List and Count Above Title Input */}
              {(currentNote.tags && currentNote.tags.length > 0) && (
                <div className="tag-list-section">
                  <span className="tag-count-label bg-label">{currentNote.tags.length} TAG{currentNote.tags.length !== 1 ? 'S' : ''}</span>
                  <div className="tag-list-main-advanced">
                    {currentNote.tags.map(tag => (
                      <div
                        key={tag}
                        className="tag-pill-main-advanced"
                      >
                        <Tag className="tag-icon" />
                        <span className="tag-label">{tag}</span>
                        <button
                          className="remove-tag-btn-main-advanced"
                          onClick={() => handleRemoveTag(tag)}
                          title="Remove tag"
                          type="button"
                        >
                          <XCircle className="remove-tag-icon" />
                        </button>
                      </div>
                    ))}
                  </div>
              ...
              <div className="note-title-section">
                <label className="note-title-label bg-label" htmlFor="note-title-input">Add title</label>
                <div className="note-title-row">
                    ...
                    className="note-title-input"
                                ...
                                />
                  <button
                    className="add-tags-btn-advanced"
                    onClick={() => setShowTagModal(true)}
                    type="button"
                  >
                    <Tag className="add-tags-btn-icon" /> Add tags
                  </button>
                </div>
              </div>
              {/* Description Label Above Editor */}
              <div className="flex justify-between items-center mt-[30px] my-[10px]">
                <label className="note-description-label bg-label" htmlFor="note-description-editor">Description</label>
                {canEdit && editor && (
                <div className="editor-toolbar">
                  <button onClick={() => editor.chain().focus().toggleBold().run()} className={`toolbar-btn ${editor.isActive('bold') ? 'active' : ''}`} title="Bold (Ctrl+B)"><Bold size={20}/></button>
                  <button onClick={() => editor.chain().focus().toggleItalic().run()} className={`toolbar-btn ${editor.isActive('italic') ? 'active' : ''}`} title="Italic (Ctrl+I)"><Italic size={20}/></button>
                  <button onClick={() => editor.chain().focus().toggleUnderline().run()} className={`toolbar-btn ${editor.isActive('underline') ? 'active' : ''}`} title="Underline (Ctrl+U)"><Underline size={20}/></button>
                  <button onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={`toolbar-btn ${editor.isActive('heading', { level: 1 }) ? 'active' : ''}`} title="Heading (Ctrl+H)"><Heading size={20}/></button>
                  <button onClick={() => editor.chain().focus().toggleHighlight().run()} className={`toolbar-btn ${editor.isActive('highlight') ? 'active' : ''}`} title="Highlight"><PaintBucket size={20}/></button>
                  <input type="color" onChange={e => editor.chain().focus().setColor(e.target.value).run()} title="Text color" className="toolbar-color-input" />
                  <button className="toolbar-btn markdown-toggle-btn" onClick={() => setShowMarkdown(prev => !prev)}>{showMarkdown ? 'Editor' : 'Preview'}</button>
                </div>
              )}
                <div className="description-row flex items-center justify-center gap-2">
                  <span className="text-gray-900 text-sm font-semibold border border-blue-400 rounded-full px-5 bg-red-200 py-1">Words: <span className="text-blue-900">{wordCount}</span></span>
                  <span className="text-gray-900 text-sm font-semibold border border-blue-400 rounded-full px-5 bg-red-200 py-1">Chars: <span className="text-blue-900">{editor ? editor.getText().length : 0}</span></span>
                </div>
              </div>
              {/* Tag Modal Popup */}
              {showTagModal && (
                <div className="tag-modal-backdrop-advanced fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm tag-modal-appear">
                  <div className="tag-modal-content-advanced bg-[#23272a] rounded-2xl shadow-2xl p-8  relative flex flex-col items-center">
                    <button
                      className="absolute top-4 right-4 text-blue-300 hover:bg-blue-500 hover:text-white rounded-full p-2 transition-all duration-200 focus:ring-2 focus:ring-blue-400"
                    ...
                    <h2 className="text-xl font-medium text-blue-200 mb-4 flex items-center gap-2 pt-[40px]"><Tag size={20} /> Manage Tags</h2>
                    {/* Tag Input */}
                    <div className="w-full flex items-center gap-2 mb-4">
                                            <input
                        className={`tag-input-modal-advanced flex-1 px-5 py-3 rounded-2xl border-2 transition-all duration-200 text-lg font-medium bg-[#181b22] text-blue-200 focus:border-blue-400 outline-none shadow-lg ${tagError ? 'border-pink-500' : 'border-blue-200'} ${tagInput ? 'has-value' : ''}`}
                                                type="text"
                                                placeholder="Add tag..."
                                                value={tagInput}
                      ...
                      />
                      <button
                        className="add-tag-fab-modal-advanced bg-blue-500 hover:bg-blue-600 text-white rounded-full py-[18px] shadow-lg transition-all"
                    ...
                    {/* Tag Suggestions Dropdown */}
                      ...
                      <div className="tag-suggestions-modal-advanced absolute left-0 right-0 mx-auto mt-2 z-30 bg-[#23272a] rounded-xl shadow-2xl border border-blue-200 w-full max-w-xs animate-popIn">
                                                    {tagSuggestions.map((sugg, idx) => (
                                                        <div
                                                            key={sugg}
                            className={`flex items-center gap-2 px-4 py-2 cursor-pointer rounded-xl transition-all ${selectedSuggestion === idx ? 'bg-blue-100 text-blue-700' : 'hover:bg-blue-50 text-blue-300'}`}
                                                            onMouseDown={() => { setTagInput(sugg); setShowSuggestions(false); setTimeout(handleAddTag, 0); }}
                                                        >
                            <Tag size={15} className="opacity-70" /> {sugg}
                                                        </div>
                                                    ))}
                                                </div>
                                            )}  
                    ...
                    {/* Live Tag List in Modal */}
                    <div className="tag-list-modal-advanced flex flex-wrap gap-2 w-full justify-center">
                    {tagError && <span className="mb-[40px] mt-[0] text-pink-500 text-base animate-fadeIn text-center w-full">{tagError}</span>}
                      {(currentNote.tags || []).map(tag => (
                        <div
                          key={tag}
                          className="tag-pill-modal-advanced flex items-center gap-2 px-4 py-2 rounded-full font-medium shadow bg-gradient-to-r from-blue-500 via-blue-400 to-blue-600 text-white relative group cursor-pointer tag-appear"
                        >
                          ...
                          <button
                            className="ml-1 bg-white/20 hover:bg-pink-500 hover:text-white rounded-full p-1 transition-all opacity-0 group-hover:opacity-100 remove-tag-btn-modal-advanced"
                            onClick={() => handleRemoveTag(tag)}
                            title="Remove tag"
                            type="button"
                          >
                            ...
                            {/* Editor Area */}
              <div id="note-description-editor" className="note-description-editor">
                {/* Copy Button */}
                <button
                  className={`absolute top-4 right-4 z-20 flex items-center gap-2 px-4 py-2 rounded-full shadow transition-all duration-200 font-medium text-base copy-btn-editor ${copied ? 'bg-green-500 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
                  title="Copy"
                  onClick={() => {
                    if (showMarkdown) {
                      const el = document.createElement('div');
                      el.innerHTML = editor ? editor.getHTML() : '';
                  ...
                  style={{ position: 'absolute', top: 16, right: 16, fontWeight: 500 }}
                >
                  {copied ? <Check size={22} className="mr-1" /> : <Copy size={22} className="mr-1" />}
                  <span>{copied ? 'Copied!' : 'Copy'}</span>
                </button>
                                {/* Markdown Preview */}
                                {showMarkdown ? (
                  <div className="w-full min-h-[300px] h-full bg-white/80 border-2 border-blue-100 rounded-2xl p-6 shadow-inner text-lg font-mono border text-blue-900 overflow-auto animate-popIn prose max-w-none" style={{ height: '100%' }}>
                                        {/* Use a markdown parser if available, else fallback to raw HTML */}
                                        <div dangerouslySetInnerHTML={{ __html: editor ? editor.getHTML() : '' }} />
                                    </div>
                                ) : (
                  <EditorContent editor={editor} className="tiptap-editor w-full min-h-[300px] h-full flex-1 bg-[#23272a] rounded-2xl p-6 shadow-inner text-lg text-blue-900 outline-none focus:ring-4 focus:ring-blue-200 transition-all duration-200 animate-popIn" />
                                )}
                                </div>
              {/* Editor Toolbar Above Description */}
